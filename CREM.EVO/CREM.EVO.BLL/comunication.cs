...
namespace CREM.EVO.BLL
{
    public class comunication
    {
        ...
        public void AddtoSend(byte[] inData, int inLen)
        {
            ...
            else
            {
                ...
                PrintCmd(sendbuf);
                //test:
                //byte[] test = new byte[sendbuf.Length * 2];
                //Array.Copy(sendbuf, 0, test, 0, sendbuf.Length);
                //Array.Copy(sendbuf, 0, test, sendbuf.Length, sendbuf.Length);
                //Sendcmd(test, test.Length);
        ...
        {
            string ret = DateTime.Now.ToFileTimeUtc().ToString()+":   Send Cmd >> ";
        ...
        private byte[] Packet2Cmd(byte[] inData, int inLen)
        {
            byte[] sendbuf = new byte[inLen + 6];
            int pos = 0;
            sendbuf[pos++] = (byte)(STX >> 8);
            sendbuf[pos++] = (byte)(STX);
            sendbuf[pos++] = (byte)((2 + inLen)>>8);
        ...
        private byte Checksum(byte[] inData, int inLen, byte packetno)
        {
            byte ret = packetno;
        ...
        public comunication()
        {
            ...
            tmr.Interval = TimeSpan.FromMilliseconds(5000);
            tmr.Tick += tmr_Tick;
        }
        ...
        void tmr_Tick(object sender, EventArgs e)
        {
            ...
            if (EVOEvent != null)
            {
                EVOEvent(this, new EVOData() {  _cmdType = CmdType.TIME_OUT, datainno=0, datainLen=0, CheckSum=0});
            }
            _InData.Clear();
        ...
        void _mySerialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            lock (m_lock)
            {
                ...
                {
                    Console.WriteLine("DataReceive Time Error!"+ex.ToString());
                }
        ...
        private void DealwithDataIn()
        {
            ...
            if (_InData.Count >= 6)
            {
                ...
                int reclen = (_InData[2]<<8) + _InData[3];
                if (reclen < _InData.Count - 4)
                {
                    _InData.Clear();
                    _crtSendState = SendState.IDEL;
                    if (IsTransfer)
    ...
    public class EVOData
    {
        ...
        public UInt16 datainLen { get; set; }
